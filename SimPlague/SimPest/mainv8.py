...
exportbestand = base / "exportdata"/"simulation_data.csv" # CSV niet heel charmant, maar minder omslachtig, bij timestamp in naam wel onhandig.... dan aanpassen met export_dir en padverwijzing
...
class Prey2(Sprite):
    def __init__(self, image, game):
        super().__init__(image, game)
        self.eatSound = load_sound_safe(eatSound1)
        self.speed = 5          # sneller
        self.energy = 80        # minder reserve
    ...
    def update(self):
        self.energy -= 0.15     # sneller energieverlies
        if self.energy <= 0:
    ...
    def eat(self, food):
        self.energy += 20       # minder energie-opbrengst
        self.eatSound.play()
...
class SurvivalSim:
    ...
    def setup_ui(self):
        self.buttons = [
            {"label": "Pause / Start", "rect": pygame.Rect(20, 20, 130, 42), "action": "toggle_pause"},
            {"label": "Reset",         "rect": pygame.Rect(165, 20, 130, 42), "action": "reset"},
            {"label": "Spawn Prey",    "rect": pygame.Rect(20, 72, 130, 42), "action": "spawn_prey"},
            {"label": "Spawn Prey2",   "rect": pygame.Rect(165, 72, 130, 42), "action": "spawn_prey2"},
            {"label": "Spawn Food",    "rect": pygame.Rect(20, 124, 130, 42), "action": "spawn_food"},
            {"label": "Spawn Pred",    "rect": pygame.Rect(165, 124, 130, 42), "action": "spawn_pred"},
        ]
    ...
    def drawPanel(self):
        ...
        self.surface.blit(title, (20, 180))
        ...
        self.surface.blit(param_title, (20, 250))

        self.param_fields = [
            {"label": "Prey speed",        "value": "3.0",                       "rect": pygame.Rect(20, 290, 275, 36)},
            {"label": "Prey2 speed",       "value": "5.0",                       "rect": pygame.Rect(20, 345, 275, 36)},
            {"label": "Predator speed",    "value": "4.0",                       "rect": pygame.Rect(20, 400, 275, 36)},
            {"label": "Food spawn chance", "value": f"{self.food_spawn_chance}", "rect": pygame.Rect(20, 455, 275, 36)},
            {"label": "Max food",          "value": f"{self.max_food}",          "rect": pygame.Rect(20, 510, 275, 36)},
            {"label": "Paused",            "value": str(self.paused),              "rect": pygame.Rect(20, 565, 275, 36)},
        ]
        ...
        self.surface.blit(stats_title, (20, 650))
    ...
    def timestep(self):
        ...
        # grafiekhistorie updaten
        self.food_history.append(len(self.foodPopulation))
        self.prey_history.append(len(self.preyPopulation))
        self.prey2_history.append(len(self.prey2Population))
        self.pred_history.append(len(self.predatorPopulation))

        # waarden wegschrijven naar csv
        self.csv_writer.writerow([self.timestep_counter,len(self.foodPopulation),len(self.preyPopulation),len(self.prey2Population),len(self.predatorPopulation)])
    ...
    def playGame(self):
        ...
        self.preyImage = pygame.transform.scale(pygame.image.load(prey1).convert_alpha(), (50, 50)) #schalen nog parametriseren!
        ...
        # CSV setup
        self.csv_file = open(exportbestand, "w", newline="")
        self.csv_writer = csv.writer(self.csv_file, delimiter=';') #nederlandse excel leest geen komma's maar ; als delimiter!
        ...
        self.csv_file.close() # CSV-bestand netjes afsluiten
        pygame.quit()
